...
fps = 10000  # Increase the frame rate to make the game run faster
...
def move_pacman(dx, dy):
    ...
    new_x = pacman_x + dx
    new_y = pacman_y + dy
    if 0 <= new_x < len(board[0]) and 0 <= new_y < len(
            board) and board[new_y][new_x] != '#':
        pacman_x = new_x
        pacman_y = new_y
...
def draw_pacman():
    ...
    pygame.draw.circle(window, YELLOW,
                       (pacman_x * block_size + block_size // 2,
                        pacman_y * block_size + block_size // 2),
                       block_size // 2)
...
def draw_enemy():
    ...
    for enemy in enemies:
        x, y = enemy
        pygame.draw.circle(window, RED, (x * block_size + block_size // 2,
